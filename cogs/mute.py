...
class Mute:
    ...
    async def mute(self, ctx, user: discord.Member, duration: str=None, *, reason: str=None):
        ...
        if user.id in self.json[server.id]:
            msg = 'User was already muted; resetting their timer...'
        elif role in user.roles:
            msg = 'User was muted but had no timer, adding it now...'
        else:
            msg = '```diff\n-{} Muted and can speak after {}\n```'.format(user.name,duration)
        ...
        if ctx.message.server.id == '195278167181754369':
            await self.bot.send_message(self.bot.get_channel('214580801461813249'),"\N{SPEAKER WITH CANCELLATION STROKE} **{}** was muted for **{} seconds** by **{}**.\nReason: `{}` \N{EYES}".format(user.name, duration, author.name, reason))

    @commands.command(pass_context=True, no_pm=True, name='listmuted')
    @checks.mod_or_permissions(manage_messages=True)
    async def list_muted(self, ctx):
        ...
        the reason for mute, if any."""
        server = ctx.message.server
        server_id = server.id
        if not (server_id in self.json and self.json[server_id]):
            await self.bot.say("No users are currently muted.")
            return
        ...
        headers = ['Member', 'Remaining', 'muted by']
        table = []
        disp_table = []
        now = time.time()
        for member_id, data in self.json[server_id].items():
            member_name = getmname(member_id)
            muter_name = getmname(data['by'])
            t = data['until']
            sort = t if t else float("inf")
            table.append((sort, member_name, t, muter_name))

        for _, name, rem, mod in sorted(table, key=lambda x: x[0]):
            remaining = _generate_timespec(rem - now) if rem else 'forever'
            disp_table.append((name, remaining, mod))
    ...
    async def warn(self, ctx, user: discord.Member, *, reason: str=None):
        ...
        if ctx.message.server.id == '195278167181754369': #diep
            await self.bot.send_message(self.bot.get_channel('214580801461813249'),'\n:warning: **{}** has been warned by **{}**. \nReason: `{}` :eyes:'.format(user.name, author.name, reason))
    ...
    async def _unmute(self, member, reason):
        ...
        if role:
            ...
            msg = 'Your mute in %s has ended.' % member.server.name
            if reason:
                msg += "\nReason was: %s" % reason
    ...
    async def on_member_update(self, before, after):
        ...
        if role and role in before.roles and role not in after.roles:
            msg = 'Your mute in %s was ended early by a moderator/admin.' % before.server.name
    ...
    async def on_member_join(self, member):
        ...
        if role:
            ...
            if duration > 0:
                await self.bot.add_roles(member, role)
                reason = 'mute re-added on rejoin. '
...
def compat_load(path):
    ...
    for server, mute in data.items():
        for user, pdata in mute.items():
