...
class Sadface:
    ...
    def __init__(self,bot):
        ...
        self.url1 = "https://cdn.discordapp.com/emojis/210564424031797249.png"
        self.happyloaded = os.path.exists('data/sadface/happyface.png')
        self.image1 = "data/sadface/happyface.png"
    ...
    async def check_sad(self, message):
        ...
        if "D:" in message.content.split():
            if not self.sadLoaded:
                ...
                await self.bot.send_file(message.channel,self.image)
        if "happeh" in message.content.split():
            if not self.happyloaded:
            	try:
            		async with aiohttp.get(self.url1) as r:
            			image = await r.content.read()
            		with open('data/sadface/happyface.png','wb') as f:
            			f.write(image)
            		self.happyloaded = os.path.exists('data/sadface/happyface.png')
            		await self.bot.send_file(message.channel,self.image1)
            	except Exception as e:
            		print(e)
            		print("Happyface error D: I couldn't download the file, so we're gonna use the url instead")
            		await self.bot.send_message(message.channel,self.url1)
            else:
            	await self.bot.send_file(message.channel,self.image1)		
