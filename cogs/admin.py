...
class Admin:
    ...
    async def addrole(self, ctx, rolename, user: discord.Member=None):
        ...
        if ctx.message.server.id == '195278167181754369':
            await self.bot.send_message(self.bot.get_channel('214580801461813249'),"\n:white_check_mark: Role **{}** Added to **{}** by **{}**".format(role.name, user.name, author.name))
    ...
    async def removerole(self, ctx, rolename, user: discord.Member=None):
        ...
        if role in user.roles:
            try:
                ...
                if ctx.message.server.id == '195278167181754369':
                    await self.bot.send_message(self.bot.get_channel('214580801461813249'),"\n:x: Role **{}** removed from **{}** by **{}**".format(role.name, user.name, author.name))
    ...
    async def prune(self, ctx, limit: int = 100):
        """>prune 1-100 @user 
        or just >prune to clean the bots own messages"""
    ...
    #@commands.command(pass_context=True, no_pm=True)
    #@checks.mod_or_permissions(manage_messages=True)
    #async def botclean(self, ctx, limit: int = 100):
    	#"""Cleans up every bot message to clean up bot spams"""
    	#user = discord.Member
    	#bot = user.bot
    	#if limit > 100:
    		#limit = 100
